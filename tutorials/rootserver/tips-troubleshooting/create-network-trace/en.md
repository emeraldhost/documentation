---
slug: "create-network-trace"
language: "en"
title: "How to Create a Network Trace with MTR"
...
short_title: "Create Network Trace"
sort: 2
...
1. **Download WinMTR**\
   Download [WinMTR](https://winmtr.net/) and start the program.

2. **Enter the server IP**\
   Enter your server IP address in the `Host` field.

3. **Start the measurement**\
   Click `Start` and wait until at least 200 packets have been sent. You can see the number of sent packets in the `Sent` column.

4. **Export the result**\
   Click `Stop` and then `Export TEXT` to save the result.
...
3. **Save the result**\
   The result is displayed directly in the terminal. Copy it or redirect it to a file:
...
3. **Save the result**\
   Copy the result or redirect it to a file:
