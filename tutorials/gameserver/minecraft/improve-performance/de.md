...
language: "de"
title: "So verbesserst Du die Performance auf Deinem Minecraft Java Edition Server"
...
> [!NOTE]
> **spark ist in Paper bereits enthalten**
>
...
1. **Passende Version herunterladen**\
   Wähle **Bukkit**, wenn Dein Server mit Paper, Purpur oder Spigot läuft, bzw. **Forge**, **NeoForge** oder **Fabric** für Mod-Server.

2. **Datei hochladen**\
   Lade die `.jar`-Datei auf Plugin-Servern in den `plugins`-Ordner hoch (siehe [Plugins installieren](/tutorials/gameserver/minecraft/install-plugins)), auf Mod-Servern in den `mods`-Ordner (siehe [Mods hinzufügen](/tutorials/gameserver/minecraft/install-mods)).
...
Die Befehle kannst Du im Spiel (mit [OP-Rechten](/tutorials/gameserver/minecraft/grant-op-rights)) oder über die Konsole in der Verwaltung ausführen.
...
> [!TIP]
> Erstelle vor größeren Änderungen an der Konfiguration ein [Backup](/tutorials/gameserver/minecraft/create-backup).

### server.properties

Die beiden wirksamsten Einstellungen stehen in der Datei `server.properties`:

- **`view-distance`** bestimmt, wie viele Chunks der Server um jeden Spieler lädt und versendet. Weniger Chunks bedeuten deutlich weniger Last – wie Du den Wert sinnvoll setzt, zeigt [View-Distance ändern](/tutorials/gameserver/minecraft/change-view-distance).
- **`simulation-distance`** bestimmt, in welchem Umkreis Mobs, Redstone und Pflanzen aktiv berechnet werden. Eine niedrigere Simulation-Distance spart oft am meisten Tickzeit – Details in [Simulation-Distance ändern](/tutorials/gameserver/minecraft/change-simulation-distance).

### Paper-Konfiguration

Läuft Dein Server mit Paper oder Purpur, kannst Du Dich per [SFTP](/tutorials/gameserver/establish-sftp-connection) verbinden und in der Datei `config/paper-world-defaults.yml` weitere Optimierungen vornehmen. Alle Einstellungen sind in der [offiziellen Paper-Dokumentation](https://docs.papermc.io/paper/reference/world-configuration) beschrieben – diese hier bringen nachweislich etwas:
...
Läuft Dein Server noch mit Vanilla und benötigst Du keine Mods, ist der Wechsel zu Paper die wirksamste einzelne Maßnahme. Die Software wechselst Du über das Feld **Software** in den Einstellungen der Verwaltung – die einzelnen Schritte findest Du in [Version ändern](/tutorials/gameserver/minecraft/change-version).
...
- **Weit verstreute Spieler**: Jeder Spieler lädt seinen eigenen Chunk-Bereich. Eine [Welt-Grenze](/tutorials/gameserver/minecraft/change-world-border) hält die Welt kompakt und verhindert, dass ständig neue Chunks generiert werden müssen.
