...
language: "en"
title: "How to Improve Performance on Your Minecraft Java Edition Server"
...
sort: 21
related: ["gameserver/minecraft/fix-server-crashes", "gameserver/minecraft/grant-op-rights", "gameserver/minecraft/increase-slots", "gameserver/minecraft/install-mods"]
---
...
2. **Upload the file**\
   On plugin servers, upload the `.jar` file to the `plugins` folder (see [Install Plugins](/tutorials/gameserver/minecraft/install-plugins)); on modded servers, upload it to the `mods` folder (see [Install Mods](/tutorials/gameserver/minecraft/install-mods)).
...
You can run the commands in-game (with [OP rights](/tutorials/gameserver/minecraft/grant-op-rights)) or via the console in the dashboard.
...
> [!TIP]
> Create a [backup](/tutorials/gameserver/minecraft/create-backup) before making bigger changes to the configuration.

### server.properties

The two most effective settings live in the `server.properties` file:

- **`view-distance`** determines how many chunks the server loads and sends around each player. Fewer chunks mean significantly less load – see [Change View Distance](/tutorials/gameserver/minecraft/change-view-distance) for sensible values.
- **`simulation-distance`** determines the radius in which mobs, redstone and plants are actively ticked. A lower simulation distance often saves the most tick time – details in [Change Simulation Distance](/tutorials/gameserver/minecraft/change-simulation-distance).

### Paper Configuration

If your server runs Paper or Purpur, you can connect via [SFTP](/tutorials/gameserver/establish-sftp-connection) and apply further optimizations in the file `config/paper-world-defaults.yml`. All settings are described in the [official Paper documentation](https://docs.papermc.io/paper/reference/world-configuration) – these ones demonstrably help:
...
If your server still runs Vanilla and you don't need mods, switching to Paper is the single most effective measure. You change the software via the **Software** field in the settings of the dashboard – the individual steps can be found in [Change Version](/tutorials/gameserver/minecraft/change-version).
...
- **Widely scattered players**: Every player loads their own chunk area. A [world border](/tutorials/gameserver/minecraft/change-world-border) keeps the world compact and prevents new chunks from being generated all the time.
